import { Package, TrendingUp, CircleCheck, Clock, ScanLine, User, FileText, History, Plus, ArrowDownNarrowWide, ArrowUpNarrowWide, AlertTriangle } from 'lucide-react';
...
export default function ReservaDetalhes({ reserva, baixas, pcpOps = [], onBaixa }) {
  ...
  const opVinculada = pcpOps.find(op => 
    op.codigo_produto && reserva.codigo_produto && 
    op.codigo_produto.toLowerCase().trim() === reserva.codigo_produto.toLowerCase().trim()
  );

  const opCancelada = opVinculada?.status === 'Cancelado';
  const quantidadeDivergente = opVinculada && opVinculada.quantidade_total > 0 && opVinculada.quantidade_total < reserva.quantidade;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50 dark:bg-white/5 p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 relative overflow-hidden group">
        ...
        <div className="space-y-2 relative z-10 w-full">
          ...
          <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic opacity-80 mb-4">
            <History className="w-3 h-3" />
            Registrado em {format(new Date(reserva.created_at), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
          </div>

          {opVinculada && (opCancelada || quantidadeDivergente) && (
            <Alert variant="destructive" className="mt-4 bg-amber-500/10 text-amber-600 dark:text-amber-500 border-amber-500/50">
              <AlertTriangle className="h-4 w-4" />
              <AlertTitle className="font-bold">Divergência Operacional Detectada (OP: {opVinculada.codigo_op})</AlertTitle>
              <AlertDescription className="text-xs">
                {opCancelada 
                  ? "A Ordem de Produção vinculada a este produto encontra-se cancelada no PCP." 
                  : `A quantidade aprovada na OP (${opVinculada.quantidade_total}) é inferior à quantidade reservada neste lote (${reserva.quantidade}).`}
              </AlertDescription>
            </Alert>
          )}
        </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <PremiumCard title="Especificações de Produção" icon={FileText} iconColor="#f59e0b" className="dark:bg-slate-900/80">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 py-4">
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Orientação da Sequência</span>
                <div className={cn(
                  "flex items-center gap-2 font-mono text-sm font-black px-3 py-2 rounded-xl border italic truncate",
                  reserva.sequencia_decrescente 
                    ? "bg-amber-500/5 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20"
                    : "bg-blue-500/5 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20"
                )}>
                  {reserva.sequencia_decrescente ? (
                    <>
                      <ArrowDownNarrowWide className="w-4 h-4" />
                      <span>Decrescente ({reserva.numero_inicial} → {reserva.numero_final})</span>
                    </>
                  ) : (
                    <>
                      <ArrowUpNarrowWide className="w-4 h-4" />
                      <span>Crescente ({reserva.numero_inicial} → {reserva.numero_final})</span>
                    </>
                  )}
                </div>
              </div>
