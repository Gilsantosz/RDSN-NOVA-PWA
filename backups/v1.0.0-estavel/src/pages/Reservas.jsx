...
import { Plus, Package, Eye, History, BarChart3, RefreshCw } from 'lucide-react';
...
export default function Reservas() {
  ...
  const { data: reservas = [], isLoading: loadingReservas } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (isAdmin && setorAtivo === 'ALL') {
        return await base44.entities.ReservaLote.list('-created_at', 500);
      }
      return await base44.entities.ReservaLote.filter({ setor_id: setorAtivo }, '-created_at', 500);
    },
    enabled: !!setorAtivo
  });
  ...
  const { data: integracoes = [] } = useQuery({
    ...
    queryFn: async () => {
      const todas = await base44.entities.IntegracaoExterna.list();
  ...
  const sincronizarMutation = useMutation({
    mutationFn: async ({ reserva_id, integracao_id }) => {
      return await base44.functions.invoke('sincronizarReserva', {
        reserva_id,
        integracao_id
      });
    },
  ...
  const createReservaMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const validacao = await base44.functions.invoke('validarIntervalosNumeracao', {
      ...
      if (!seq) {
        seq = await base44.entities.SequenciaAnual.create({
          letra_produto: data.letra_produto,
          ano: data.ano,
          setor_id: setorAtivo,
          ultimo_numero: 0,
      ...
      const numeracoesLivres = await base44.entities.NumeracaoLivre.filter({
        letra_produto: data.letra_produto,
        ano: data.ano,
        setor_id: setorAtivo,
        disponivel: true
      ...
      for (const livre of numeracoesLivres) {
        ...
        if (temSobreposicao) {
          ...
          if (numeroInicialReserva === numeroInicialLivre && numeroFinalReserva === numeroFinalLivre) {
            await base44.entities.NumeracaoLivre.update(livre.id, { disponivel: false });
          }
          ...
          // CASO 3: Reserva usa parte do início - ATUALIZAR números restantes
            ...
            await base44.entities.NumeracaoLivre.update(livre.id, {
          ...
          // CASO 5: Reserva usa o MEIO - DIVIDIR em duas partes
            ...
            await base44.entities.NumeracaoLivre.create({
      ...
      await base44.entities.SequenciaAnual.update(seq.id, {
  ...
  const createBaixaMutation = useMutation({
    mutationFn: async (data) => {
      const baixa = await base44.entities.BaixaLote.create(data);
      ...
      await base44.entities.ReservaLote.update(data.reserva_id, {
  ...
  const encurtarLoteMutation = useMutation({
    mutationFn: async ({ reserva, novoNumeroFinal, observacoes, numeracaoLivre }) => {
      ...
      await base44.entities.ReservaLote.update(reserva.id, {
  ...
  const quebrarLoteMutation = useMutation({
    mutationFn: async ({ reserva, quantidadePorLote, observacoes, lotes }) => {
      ...
      for (let i = 1; i < lotes.length; i++) {
        const lote = lotes[i];
        const novaReserva = await base44.entities.ReservaLote.create({
          letra_produto: reserva.letra_produto,
          ano: reserva.ano,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      ...
      // Se houver numeração não baixada, liberar para reutilização
      ...
      if (quantidadeNaoUsada > 0) {
        ...
        await base44.entities.NumeracaoLivre.create({
          ...
          motivo: 'CANCELAMENTO',
          disponivel: true,
          observacoes: `Reserva deletada - ${reserva.codigo_completo}`
        });
      }

      await base44.entities.ReservaLote.delete(id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reservas'] });
      queryClient.invalidateQueries({ queryKey: ['numeracoes-livres'] });
      toast.success('Reserva deletada e numeração liberada!');
    }
  });
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status, acao }) => {
      const reserva = reservas.find(r => r.id === id);
      await base44.entities.ReservaLote.update(id, { status });
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="w-[95vw] md:max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl p-4 md:p-6 dark:bg-slate-900 dark:border-slate-800">
            <DialogHeader className="pb-2 border-b border-slate-100 dark:border-slate-800">
              <DialogTitle className="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                <span className="w-8 h-8 rounded-xl bg-slate-900 dark:bg-slate-800 flex items-center justify-center"><Plus className="w-4 h-4 text-white" /></span>
        ...
        <Dialog open={showBaixa} onOpenChange={setShowBaixa}>
          <DialogContent className="w-[95vw] md:max-w-xl max-h-[90vh] overflow-y-auto rounded-2xl p-4 md:p-6 dark:bg-slate-900 dark:border-slate-800">
            <DialogHeader className="pb-2 border-b border-slate-100 dark:border-slate-800">
              <DialogTitle className="text-xl font-bold text-slate-900 dark:text-slate-100">Registrar Baixa</DialogTitle>
            </DialogHeader>
            {selectedReserva && (
              <BaixaForm
        ...
        <Dialog open={showDetalhes} onOpenChange={setShowDetalhes}>
          <DialogContent className="w-[95vw] md:max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl p-4 md:p-6 dark:bg-slate-900 dark:border-slate-800">
            <DialogHeader className="pb-2 border-b border-slate-100 dark:border-slate-800">
              <DialogTitle className="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
        ...
        <Dialog open={showHistorico} onOpenChange={setShowHistorico}>
          <DialogContent className="w-[95vw] md:max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl p-4 md:p-6 dark:bg-slate-900 dark:border-slate-800">
            <DialogHeader className="pb-2 border-b border-slate-100 dark:border-slate-800">
              <DialogTitle className="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            ...
            {selectedReserva && (
              <div className="space-y-4">
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg">
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">Reserva selecionada:</p>
                  <p className="font-semibold text-slate-900 dark:text-slate-100">{selectedReserva.codigo_completo}</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400">{selectedReserva.cliente}</p>
                </div>

                <div className="space-y-2">
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Selecione a integração:</p>
                  {integracoes.length === 0 ? (
                    <p className="text-sm text-slate-500">Nenhuma integração ativa encontrada</p>
                  ) : (
                    <div className="space-y-2">
                      {integracoes.map(int => (
                        <Button
