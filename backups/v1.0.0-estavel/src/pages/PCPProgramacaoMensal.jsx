...
import { Plus, ChevronLeft, ChevronRight, BarChart3, Zap, Pencil, SlidersHorizontal, RefreshCw, Factory } from 'lucide-react';
...
export default function PCPProgramacaoMensal() {
  ...
  const { data: ops = [] } = useQuery({
    ...
    queryFn: () => {
      if (!setorAtivo || setorAtivo === 'ALL') {
        return base44.entities.PCPOrdemProducao.filter({ mes, ano, status: 'Ativo' }, 'item_num', 500);
      }
      return base44.entities.PCPOrdemProducao.filter({ mes, ano, status: 'Ativo', setor_id: setorAtivo }, 'item_num', 500);
    },
  ...
  const { data: opsAnterior = [] } = useQuery({
    ...
    queryFn: () => {
      if (!setorAtivo || setorAtivo === 'ALL') {
        return base44.entities.PCPOrdemProducao.filter({ mes: mesAnt, ano: anoAnt, status: 'Ativo' }, 'item_num', 500);
      }
      return base44.entities.PCPOrdemProducao.filter({ mes: mesAnt, ano: anoAnt, status: 'Ativo', setor_id: setorAtivo }, 'item_num', 500);
    },
    enabled: !!setorAtivo
  });
  ...
  const { data: producoesMesAnterior = [] } = useQuery({
    ...
    queryFn: () => {
      if (!setorAtivo || setorAtivo === 'ALL') {
        return base44.entities.PCPProducaoDiaria.filter({ mes: mesAnt, ano: anoAnt }, null, 5000);
      }
      return base44.entities.PCPProducaoDiaria.filter({ mes: mesAnt, ano: anoAnt, setor_id: setorAtivo }, null, 5000);
    },
  ...
  React.useEffect(() => {
    ...
    base44.functions.invoke('sincronizarBaixasComPCP', { mes, ano, setor_id: setorAtivo !== 'ALL' ? setorAtivo : null })
  ...
  React.useEffect(() => {
    const unsubscribe = base44.entities.PCPProducaoDiaria.subscribe(() => {
  ...
  const updateProducaoMutation = useMutation({
    mutationFn: async ({ op_id, dia, campo, valor }) => {
      ...
      if (existing) {
        ...
        return base44.entities.PCPProducaoDiaria.create({ op_id, mes, ano, dia, [campo]: Number(valor) || 0, setor_id: setorAtivo || '' });
      }
    },
  ...
  const handleSincronizarBaixas = async () => {
    ...
    try {
      const res = await base44.functions.invoke('sincronizarBaixasComPCP', { mes, ano, setor_id: setorAtivo !== 'ALL' ? setorAtivo : null });
      queryClient.invalidateQueries({ queryKey: ['pcp-producoes', mes, ano, setorAtivo] });
  ...
  const content = (
    <div className="p-4 md:p-6 min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      {/* Tabela Principal */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow overflow-hidden transition-colors">
      ...
      {/* Resumo Mensal Premium */}
      <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 mb-12">
        {[
          { label: 'Total Previsto', value: resumo.totalPrev.toLocaleString(), color: 'blue' },
          { label: 'Total Realizado', value: resumo.totalReal.toLocaleString(), color: 'emerald' },
          { label: 'Saldo Mensal', value: resumo.saldoTotal >= 0 ? `+${resumo.saldoTotal}` : resumo.saldoTotal.toLocaleString(), color: resumo.saldoTotal >= 0 ? 'emerald' : 'rose' },
          { label: 'Média Diária', value: resumo.mediaDiaria, color: 'indigo' },
          { label: '% Atendimento', value: `${resumo.perc}%`, color: Number(resumo.perc) >= 90 ? 'emerald' : Number(resumo.perc) >= 70 ? 'amber' : 'rose' },
          { label: 'OPs Ativas', value: ops.length, color: 'slate' },
        ].map((item, i) => (
          <Card key={i} className="relative overflow-hidden group border-slate-200 dark:border-white/5 bg-white dark:bg-slate-900/40 backdrop-blur-xl transition-all hover:scale-[1.02] active:scale-95 shadow-lg">
            <CardContent className="p-5">
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] italic opacity-80 mb-1">{item.label}</p>
              <p className={cn(
