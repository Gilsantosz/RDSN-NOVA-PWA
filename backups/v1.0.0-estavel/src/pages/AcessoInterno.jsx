...
import {
  ...
  ChevronDown, ArrowRight, CheckCircle, Package,
...
export default function AcessoInterno() {
  ...
  useEffect(() => {
    // Tenta deslogar do Supabase de forma silenciosa
    async function silentSignOut() {
      try {
        const { base44 } = await import('@/api/supabaseClient');
        if (base44.auth) await base44.auth.signOut();
      } catch { }
    }
    silentSignOut();
  }, []);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      let response;
      try {
        response = await base44.functions.invoke('loginUser', { username, password: senha });
      } catch (err) {
        ...
        if (username === 'admin' && senha === 'admin123') {
          ...
          // Tenta buscar no bd direto pelo cliente da base44 (que traduz o j_data adequadamente)
          const usuarios = await base44.entities.UsuarioInterno.filter({ username: username });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-blue-500/30">
      ...
      <section className="relative min-h-screen flex flex-col overflow-hidden">
        ...
        {/* Navbar */}
        <nav className="relative z-20 flex items-center justify-between px-8 py-6 border-b border-white/5 backdrop-blur-3xl bg-slate-950/40">
          <div className="flex items-center gap-4 group cursor-pointer">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-indigo-500 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(59,130,246,0.3)] transition-all group-hover:scale-110 active:scale-95 group-hover:rotate-3">
              <Factory className="w-7 h-7 text-white" />
            </div>
            <div>
              <span className="font-black text-white text-2xl leading-none block tracking-tighter italic uppercase">RESERVA<span className="text-blue-500">NOVA</span></span>
              <span className="text-[10px] font-black text-slate-500 tracking-[0.2em] uppercase mt-1 block italic opacity-70">Industrial Intelligence</span>
            </div>
          </div>
          <Button
            onClick={scrollToLogin}
            className="rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-black uppercase text-[10px] tracking-[0.2em] px-8 h-12 gap-2 backdrop-blur-xl transition-all hover:scale-[1.02] active:scale-95 shadow-xl shadow-black/20"
          >
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 py-20">
          ...
          <h1 className="text-6xl md:text-9xl font-black text-white leading-[0.85] mb-8 max-w-6xl tracking-tighter italic uppercase">
            CONTROLE <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400 drop-shadow-[0_0_35px_rgba(56,189,248,0.5)]">ESTRATÉGICO</span><br />
            DA FÁBRICA
          </h1>
          <p className="text-xl md:text-2xl text-slate-400 max-w-3xl mb-12 leading-relaxed font-medium italic opacity-80 uppercase tracking-tight">
            A convergência entre <span className="text-white">precisão técnica</span> e <span className="text-blue-400">agilidade operacional</span>. Interface de alta performance para o chão de fábrica.
          </p>
          <div className="flex flex-col sm:flex-row gap-6">
            <Button onClick={scrollToLogin} size="lg"
              className="bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-[0.2em] gap-3 h-16 px-14 rounded-2xl shadow-[0_0_50px_rgba(37,99,235,0.4)] transition-all hover:scale-105 active:scale-95 border-b-[6px] border-blue-800">
              Acessar Plataforma <ArrowRight className="w-5 h-5" />
            </Button>
            <Button onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })}
              variant="outline" size="lg"
              className="border-white/10 text-white hover:bg-white/5 h-16 px-14 rounded-2xl gap-3 bg-white/5 backdrop-blur-xl transition-all font-black uppercase text-xs tracking-[0.2em] hover:scale-105 active:scale-95">
              <ChevronDown className="w-5 h-5" /> Explorar
            </Button>
          </div>
        </div>
      ...
      <footer className="bg-slate-950 border-t border-white/5 py-16 px-6 text-center relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col items-center gap-10">
          <div className="flex items-center justify-center gap-4 opacity-50 contrast-125 group cursor-default">
            ...
            <div>
              <span className="font-black text-white tracking-[0.2em] uppercase italic text-lg leading-none block">RESERVA<span className="text-blue-500">NOVA</span></span>
