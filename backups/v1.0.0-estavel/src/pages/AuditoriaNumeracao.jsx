...
export default function AuditoriaNumeracao() {
  ...
  const { data: numeracoesLivres = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!setorAtivo) return [];
      if (isAdmin && setorAtivo === 'ALL') {
        return await base44.entities.NumeracaoLivre.list('-created_at', 500);
      }
      return await base44.entities.NumeracaoLivre.filter({ setor_id: setorAtivo }, '-created_at', 500);
    },
    enabled: !!letraSelecionada && !!anoSelecionado && !!setorAtivo
  });
  ...
  const { data: regras = [] } = useQuery({
    ...
    queryFn: () => base44.entities.RegraAuditoria.list()
  });

  const { data: historico = [] } = useQuery({
    queryKey: ['historico-auditoria'],
    queryFn: () => base44.entities.HistoricoAuditoria.list('-created_at', 50)
  });
  ...
  const criarRegraMutation = useMutation({
    mutationFn: (dados) => base44.entities.RegraAuditoria.create(dados),
    onSuccess: () => {
  ...
  const deletarRegraMutation = useMutation({
    mutationFn: (id) => base44.entities.RegraAuditoria.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['regras-auditoria'] });
  ...
  const toggleRegraMutation = useMutation({
    mutationFn: ({ id, ativa }) => base44.entities.RegraAuditoria.update(id, { ativa }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['regras-auditoria'] });
    }
  });
  ...
  const handleAnalisar = async () => {
    ...
    setLoading(true);
    try {
      const response = await base44.functions.invoke('executarAuditoriaAutomatica', {
