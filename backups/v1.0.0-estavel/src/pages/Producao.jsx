...
import { ScanLine, Package, CalendarDays, Activity } from 'lucide-react';
  ...
  const { data: reservas = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (isAdmin && setorAtivo === 'ALL') {
        return await base44.entities.ReservaLote.filter(
        ...
        {modoProducaoDia ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              {[
                { label: 'Em Produção', value: stats.emProducao, subtitle: 'lotes ativos na fila', icon: ScanLine, color: 'blue' },
                { label: 'Reservados', value: stats.reservados, subtitle: 'lotes aguardanto início', icon: Package, color: 'amber' },
                { label: 'Total Pendente', value: stats.totalPendente.toLocaleString(), subtitle: 'unidades a coletar', icon: Activity, color: 'emerald' },
              ].map((item, i) => (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-4">
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            "w-10 h-10 rounded-xl flex items-center justify-center border transition-transform group-hover:rotate-6 shadow-lg",
                        ...
                        <div>
                          <h3 className="text-4xl font-black text-slate-900 dark:text-white italic tracking-tighter leading-none">{item.value}</h3>
                          <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest italic opacity-60 mt-2">{item.subtitle}</p>
                        </div>
                      </div>
                    </div>
                  </CardContent>
            ...
            <Card className="border border-slate-200 dark:border-0 shadow-2xl bg-white dark:bg-slate-900/40 backdrop-blur-xl overflow-hidden rounded-[2.5rem]">
              <CardHeader className="bg-slate-50 dark:bg-slate-950/40 border-b border-slate-200 dark:border-white/5 p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-blue-100 dark:bg-blue-500/20 rounded-2xl flex items-center justify-center border border-blue-200 dark:border-blue-500/20 shadow-inner">
                    <ScanLine className="w-6 h-6 text-blue-600 dark:text-blue-400" />
                  </div>
                  <CardTitle className="text-xl font-black text-slate-900 dark:text-white tracking-tighter uppercase italic leading-none">Fila Industrial de Lotes</CardTitle>
                </div>
              </CardHeader>
            ...
            <Dialog open={showBaixa} onOpenChange={setShowBaixa}>
              <DialogContent className="max-w-xl dark:bg-slate-900 dark:border-slate-800">
                <DialogHeader>
                  <DialogTitle className="dark:text-slate-100">Registrar Baixa</DialogTitle>
                </DialogHeader>
                {selectedReserva &&
                  <BaixaForm
