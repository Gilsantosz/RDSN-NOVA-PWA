...
export default function ModoProducaoDia({ reservas, produtos }) {
  ...
  const { data: sessoesAbertas = [], isLoading: loadingSessoes } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await base44.entities.ProducaoDia.filter(filtro, '-created_at', 20);
    },
    enabled: !!setorAtivo
  });
  ...
  const { data: lotesAbertos = [], isLoading: loadingLotes } = useQuery({
    ...
    queryFn: async () => {
      ...
      const todosLotes = await Promise.all(
        sessaoIds.map(id => base44.entities.ProducaoDiaLote.filter({ producao_dia_id: id }))
      );
  ...
  const { data: baixas = [] } = useQuery({
    ...
    queryFn: () => base44.entities.BaixaLote.list('-created_at', 500),
  });
  ...
  const iniciarProducaoMutation = useMutation({
    mutationFn: async (lotes) => {
      ...
      // Criar sessão
      const sessao = await base44.entities.ProducaoDia.create({
        data_producao: hoje,
      ...
      await Promise.all(lotes.map(l =>
        base44.entities.ProducaoDiaLote.create({
  ...
  const fecharLoteMutation = useMutation({
    mutationFn: async ({ lote, numFinal }) => {
      ...
      const baixa = await base44.entities.BaixaLote.create({
      ...
      if (produto) {
        ...
        await base44.entities.Produto.update(produto.id, { estoque_atual: estoqueNovo });
      ...
      await base44.entities.ProducaoDiaLote.update(lote.id, {
      ...
      const todosLotesSessao = await base44.entities.ProducaoDiaLote.filter({ producao_dia_id: lote.producao_dia_id });
      const todosAbertos = todosLotesSessao.filter(l => l.status === 'ABERTO');
      if (todosAbertos.length === 0) {
        await base44.entities.ProducaoDia.update(lote.producao_dia_id, {
          status: 'FECHADA',
  ...
  const cancelarLoteMutation = useMutation({
    mutationFn: async (lote) => {
      await base44.entities.ProducaoDiaLote.update(lote.id, { status: 'CANCELADO' });
