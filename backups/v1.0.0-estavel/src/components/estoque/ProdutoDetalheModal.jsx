...
import { 
  Package, TrendingUp, TrendingDown, AlertCircle, Save, 
  Pencil, X, Clock, BarChart3 
} from 'lucide-react';
...
export default function ProdutoDetalheModal({ produto, open, onOpenChange }) {
  ...
  const { data: setor } = useQuery({
    ...
    queryFn: () => base44.entities.Setor.filter({ id: produto.setor_id }),
    enabled: !!produto?.setor_id && open,
    select: (data) => data?.[0] || null
  });

  const updateMutation = useMutation({
    mutationFn: (data) => base44.entities.Produto.update(produto.id, data),
    onSuccess: () => {
