...
export async function sincronizarBaixasComPCP(payload) {
  try {
    ...
    const ops = await base44.entities.PCPOrdemProducao.filter(filtroOps, null, 200);
    ...
    const [reservasPorCodigoArr, todasProducoesMes] = await Promise.all([
      Promise.all(
        codigosUnicos.map(codigo =>
          base44.entities.ReservaLote.filter({ codigo_produto: codigo }, null, 500)
            .then(reservas => ({ codigo, reservas }))
            .catch(() => ({ codigo, reservas: [] }))
        )
      ),
      base44.entities.PCPProducaoDiaria.filter({ mes, ano }, null, 5000)
    ]);
      ...
      await Promise.allSettled(
        registrosOrfaos.map(p => {
          ...
          if (correto && p.realizado > 0 && (!correto.realizado || correto.realizado === 0)) {
            return base44.entities.PCPProducaoDiaria.update(correto.id, { realizado: p.realizado })
              .then(() => base44.entities.PCPProducaoDiaria.delete(p.id));
          }
          return base44.entities.PCPProducaoDiaria.delete(p.id);
        })
      );
    }
    ...
    const baixasPorReservaArr = await Promise.all(
      [...reservaIdSet].map(reservaId =>
        base44.entities.BaixaLote.filter({ reserva_id: reservaId }, null, 1000)
      ...
      opsComCodigo.map(async (op) => {
        ...
        const writes = Object.entries(qtdPorDia).map(([diaStr, qtd]) => {
          ...
          if (existente) {
            ...
            return base44.entities.PCPProducaoDiaria.update(existente.id, { realizado: qtd });
          } else {
            return base44.entities.PCPProducaoDiaria.create({
