...
export async function alocarNumerosAutomatico(payload) {
  try {
    ...
    let sequencias = [];
    try {
      const todas = await base44.asServiceRole.entities.SequenciaAnual.list();
    ...
    if (sequencias.length === 0) {
      try {
        sequencia = await base44.asServiceRole.entities.SequenciaAnual.create({
          letra_produto,
          ano,
          setor_id,
          ultimo_numero: 0,
    ...
    const todasReservas = await base44.asServiceRole.entities.ReservaLote.filter({
      letra_produto,
      ano,
      setor_id
    });
    ...
    try {
      const todasNumLivres = await base44.asServiceRole.entities.NumeracaoLivre.list();
    ...
    let bloqueiosAtivos = [];
    try {
      const todosBloqueios = await base44.asServiceRole.entities.BloqueioIntervalo.list();
    ...
    if (verificarIntervaloLivre(proximoNumero, proximoNumero + quantidade - 1)) {
      const intervalo = {
        ...
        quantidadeDisponivel: quantidade,
        motivo: 'Continuação sequencial após último número'
      };
      intervalo.score = calcularScoreQualidade(intervalo);
      intervalosDisponiveis.push(intervalo);
    }
